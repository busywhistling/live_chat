...
document
	.querySelector<HTMLInputElement>("#current-message")
	?.addEventListener("change", function () {
...
document
	.querySelector<HTMLButtonElement>("#button-send-message")
...
async function sendMessage() {
	...
	if (currentMessage !== "") {
		...
		messageList.push(
			`<p>${messageData.author} (${messageData.time}): ${messageData.message}</p>`,
		);
		showMessages();
	}
}
...
function showMessages() {
	...
	const objDiv = document.querySelector<HTMLDivElement>("#chat-body")!;
